...
**Purpose:** Accept bulk data dumps in many formats and process them through the analysis pipeline.
...
**Security Implications:** Safe input handling prevents malformed/malicious data from corrupting the system while preserving evidence chain.
...
**Purpose:** Support both single-run CLI workflows and long-running server analysis with a job submission API.
