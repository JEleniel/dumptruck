...
flowchart LR
  ...
  API1 --> JobQueue[("⏳ Work Queue<br/>(In-Process)")]
  API2 --> JobQueue
  API3 --> JobQueue
  ...
  Worker1 --> Storage[("🗄️ SQLite<br/>(Primary DB File)")]
  Worker2 --> Storage
  Worker3 --> Storage
  WorkerN --> Storage
...
- **Work Queue**: In-process coordination for background tasks
...
- **Export/import snapshots**: SQLite snapshot export for backups and portability
- **Audit Logging**: SQLite audit tables (chain of custody and analysis history)
...
- SQLite + Ollama + Dumptruck server
...
- Persistent volumes for the SQLite database file
