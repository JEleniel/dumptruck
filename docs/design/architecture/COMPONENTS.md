...
### 3. Format Detection & Record Reading (src/safe_ingest.rs, src/adapters.rs)
...
### 5. Safe Read & Validation (src/safe_ingest.rs)
...
- Storage: `anomaly_scores` table with (record_id, anomaly_type, score 0-100, details JSON stored as TEXT)
...
- Background Thread (Server Mode): Continuous HIBP enrichment without blocking analysis
...
- Backends: SQLite database file (dumptruck.db); export/import via SQLite snapshots
...
- Trait-based: FormatAdapter, AnomalyDetector
...
- Health: Service startup status, SQLite/Ollama availability checks
