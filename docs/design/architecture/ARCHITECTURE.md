...
- Analyze and normalize large, heterogeneous bulk data dumps
...
3. **Format Detection**: Format detection and streaming read
4. **Chain of Custody**: ED25519 cryptographic signing with operator/timestamp/action/hash tracking
5. **Safe Read**: Binary detection, UTF-8 validation with lossy recovery, size limits
...
12. **Detection**: PII/NPI detection (16+ types), weak password detection, Risk Score (0-100)
13. **Storage**: SQLite (dumptruck.db) with history, analysis artifacts, and Chain of Custody records
...
- **External Systems**: SQLite, Ollama (optional), Have I Been Pwned API (optional)
...
- Analyze → Normalization → Deduplication → Enrichment → Detection → Storage/History
...
- Long-running service with REST analysis submission and status endpoints
...
- **Storage Layer**: Persistent state stored in a single SQLite database file; optional export/import of SQLite snapshots
...
- REST endpoints for analysis submission and status checks
...
- Composable stages: Analyze → Normalize → Dedup → Enrich → Detect → Store

### Storage (src/database.rs)

- SQLite-backed storage for analysis artifacts and hashed history
- Export/import workflow for portable SQLite snapshots
...
User Input (File/API)
    ↓
[Analyze] Format detection, streaming read
    ↓
[Safe Read] UTF-8 validation, binary detection, size checks
    ↓
[Normalization] Unicode normalization, case-folding
    ↓
[Deduplication] Hash lookup + optional similarity hints
    ↓
[Enrichment] Ollama embeddings + HIBP lookup
    ↓
[Detection] PII detection, pattern recognition
    ↓
[Storage] SQLite insert/update with audit logging
    ↓
