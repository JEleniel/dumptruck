...
use crate::npi_detection::{PiiType, detect_pii};
use crate::rainbow_table::is_weak_password_hash;

/// Detection results for a single row
#[derive(Debug, Clone)]
pub struct DetectionResult {
	/// Detected PII/NPI types found in this row
...
pub fn detect_row(row: &[String], headers: Option<&[String]>, row_index: usize) -> DetectionResult {
	...
	for (col_index, value) in row.iter().enumerate() {
		...
		result.pii_detected.extend(pii_types.clone());
...
mod tests {
	...
	fn test_aggregate_results() {
		let detections = vec![
			DetectionResult {
				pii_detected: vec![PiiType::Email],
				has_weak_password: false,
			...
			DetectionResult {
				pii_detected: vec![PiiType::Email, PiiType::PhoneNumber],
