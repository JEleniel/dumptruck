...
fn test_fingerprint_bcrypt() {
	let result =
		identify_hash_fingerprint("$2a$10$N9qo8uLOickgx2ZMRZoMyeIjZAgcg7b3XeKeUxWdeS86E36MM32Oi");
	assert_eq!(result.algorithm, HashAlgorithmFingerprint::Bcrypt);
	assert!(!result.is_weak);
	assert!(result.description.contains("Bcrypt"));

	let result =
		identify_hash_fingerprint("$2b$12$R9h/cIPz0gi.URNNX3kh2OPST9/PgBkqquzi.Ss7KIUgO2t0jWMUW");
	assert_eq!(result.algorithm, HashAlgorithmFingerprint::Bcrypt);
	assert!(!result.is_weak);

	let result =
		identify_hash_fingerprint("$2y$10$S9sBbWcp.YdJUVJvVCN/uOKlxY2GiY0l6DQ4y.xQmGj9Z3Z7G8p7m");
	assert_eq!(result.algorithm, HashAlgorithmFingerprint::Bcrypt);
	assert!(!result.is_weak);
}
...
fn test_fingerprint_weak_vs_strong() {
	...
	let salted = vec![
		identify_hash_fingerprint("$2a$10$N9qo8uLOickgx2ZMRZoMyeIjZAgcg7b3XeKeUxWdeS86E36MM32Oi"), // Bcrypt
		identify_hash_fingerprint(
			"$7$C6..../....SodiumChloride$kBGj9fHzHvFQMKkF4aBNYV4ZYvnP4q7Z/vL5Z6Z5Z6Z",
		), // Scrypt
		identify_hash_fingerprint(
