...
use crate::{
	adapters::FormatAdapter, enrichment::EnrichmentPlugin, normalization, storage::StorageAdapter,
};

/// Pipeline wires together adapter, enricher, and storage.
pub struct Pipeline<A: FormatAdapter, E: EnrichmentPlugin, S: StorageAdapter> {
	adapter: A,
	enricher: E,
	storage: S,
}

impl<A: FormatAdapter, E: EnrichmentPlugin, S: StorageAdapter> Pipeline<A, E, S> {
	...
	pub fn ingest(self, input: &str) -> Result<S, std::io::Error> {
		...
		// store a metadata row for the file; use file_sha256 as `file_id`
		...
		// helper to append file_id to every row we persist
		...
		// If first row looks like a header, use it to detect columns
		let mut header: Option<Vec<String>> = None;
		let mut expected_columns: Option<usize> = None;
		if rows.len() > 0 {
			// Heuristic: treat first row as header if any cell contains alphabetic letters and not
			// digits-only
			let first = &rows[0];
		...
		for (idx, row) in rows.iter().enumerate() {
			// skip header row from data if we treated it as header
			...
			// If we know the expected number of columns (from header), and
			// the normalized row doesn't match, treat it as a malformed row.
			if let Some(expected) = expected_columns {
				if normalized.len() != expected {
					// store a metadata row describing the malformed input and skip
					let raw = row.join(",");
			...
			// detect address/credential columns
			let mut addr_hashes: Vec<String> = Vec::new();
			let mut cred_hashes: Vec<String> = Vec::new();
			...
			if let Some(ref h) = header {
				for (i, col_name) in h.iter().enumerate() {
					...
					if i < normalized.len() {
						...
						{
							// Check if the credential is already a hash (not raw plaintext)
				...
				// fallback: scan fields for '@' as address
				for val in normalized.iter() {
					if val.contains('@') {
						addr_hashes.push(hash_utils::sha256_hex(val));
					}
					// simple credential heuristic: contains ':' or has 'pass' substring
					if val.contains(':') || val.to_lowercase().contains("pass") {
						// Check if the credential is already a hash
						if hash_utils::is_credential_hash(val) {
							has_hashed_credentials = true;
						}
			...
			// If row contains only hashed credentials (no actual plaintext), discard it
			...
			// For each address/credential pair, determine if this is a new
			// address or a known address with a new credential. Record events
			// and maintain an address->credential mapping to support later
			// detection.
			for addr in addr_hashes.iter() {
				...
				for cred in cred_hashes.iter() {
					...
					if !assoc {
						...
						// persist the association so future ingests know about it by
						// storing a typed mapping row which Postgres will extract.
			...
			// now enrich original normalized row and store (the enricher may append checksum)
			let mut enriched = self.enricher.enrich(&normalized);
			// compute a deterministic row hash from the normalized fields for deduplication
			let row_join = normalized.join("|");
			let row_hash = hash_utils::sha256_hex(&row_join);
			// append discovered hash fields for traceability
			...
			// Deduplicate: if we've already stored this normalized row (by row_hash),
			// record a duplicate event and skip storing the enriched row body.
			if storage.contains_hash(&row_hash)? {
				...
				// append row_hash for provenance and store
...
mod tests {
	...
	use super::*;
	use crate::{adapters::CsvAdapter, enrichment::ChecksumEnricher, storage::StorageAdapter};
	...
	fn pipeline_boxed_objects() {
		use crate::{adapters::CsvAdapter, enrichment::ChecksumEnricher, storage::FsStorage};
		...
		for row in rows.iter() {
			let normalized = normalization::normalize_row(row);
			let enriched = enr.enrich(&normalized);
			fs.store_row(&enriched).expect("store");
		}

		let content = FsStorage::read_all(&path).expect("read back");
		assert!(content.contains("alice") && content.contains("checksum:"));
